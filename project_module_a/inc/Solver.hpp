...
#pragma omp parallel for collapse(2) default(none) shared(Outer1, Outer2, worker)
    ...
    // with lecture BCs (Auteri, Lec 5, slides on velocity BCs).
    // =============================================================
...
#pragma omp parallel for collapse(2) default(none) shared(solve_line_for_component)
    ...
    // Lecture-style BCs + known faces (same philosophy as solve_x_only)
    // =============================================================
...
#pragma omp parallel for collapse(2) default(none) shared(solve_line_for_component)
    ...
    // Lecture-style BCs + known faces (same philosophy as solve_x_only/solve_y_only)
    // =============================================================
...
#pragma omp parallel for collapse(2) default(none) shared(solve_line_for_component)
    ...
    void solve_x_only_delta(VectorVariable &rhs_delta,
                            VectorVariable &delta,
                            Real t_old,
                            Real t_stage,
                            bool use_omp = false)
    {
        ...
        auto ub_delta = [&](int comp, Real x, Real y, Real z) -> Real
        {
            if (comp == 0)
                return u_boundary.value<0>(x, y, z, t_stage) - u_boundary.value<0>(x, y, z, t_old);
            if (comp == 1)
                return u_boundary.value<1>(x, y, z, t_stage) - u_boundary.value<1>(x, y, z, t_old);
            return u_boundary.value<2>(x, y, z, t_stage) - u_boundary.value<2>(x, y, z, t_old);
        };
        ...
        auto fill_known_face_x = [&](Dim j, Dim k, int comp)
        {
            ...
            for (Dim i = 0; i < Nx; ++i)
            {
                ...
                if (comp == 0)
                    delta.set(0, i, j, k) = ub_delta(0, x_u, y_u, z_u);
                if (comp == 1)
                    delta.set(1, i, j, k) = ub_delta(1, x_vw, y_v, z_u);
                if (comp == 2)
                    delta.set(2, i, j, k) = ub_delta(2, x_vw, y_u, z_w);
            }
        };

        auto solve_line = [&](Dim j, Dim k, int comp)
        {
            std::vector<Real> a(N, 0.0), b(N, 0.0), c(N, 0.0), d(N, 0.0), x(N, 0.0);
            ...
            for (Dim i = 1; i < N - 1; ++i)
            {
                ...
                d[i] = rhs_delta.value(comp, i, j, k);
            }

            // -------- LEFT boundary x=0 (delta form) --------
            {
                a[0] = 0;
                b[0] = 1;
                c[0] = 0;
                ...
                if (comp == 0)
                {
                    // delta u_half = delta u(0) + delta(dudx)*dx/2
                    const Real du0 = ub_delta(0, Real(0.0), y, z);

                    const Real dv_plus = ub_delta(1, Real(0.0), (Real(j) + Real(0.5)) * dy, z);
                    const Real dv_minus = ub_delta(1, Real(0.0), (Real(j) - Real(0.5)) * dy, z);
                    const Real ddv_dy = (dv_plus - dv_minus) / dy;

                    const Real dw_plus = ub_delta(2, Real(0.0), y, (Real(k) + Real(0.5)) * dz);
                    const Real dw_minus = ub_delta(2, Real(0.0), y, (Real(k) - Real(0.5)) * dz);
                    const Real ddw_dz = (dw_plus - dw_minus) / dz;

                    const Real ddudx0 = -(ddv_dy + ddw_dz);

                    d[0] = du0 + ddudx0 * (dx * Real(0.5));
                }
                else if (comp == 1)
                {
                    d[0] = ub_delta(1, Real(0.0), (Real(j) + Real(0.5)) * dy, z);
                }
                else
                {
                    d[0] = ub_delta(2, Real(0.0), y, (Real(k) + Real(0.5)) * dz);
                }
            }

            // -------- RIGHT boundary x=Lx (delta form) --------
            if (comp == 0)
            {
                a[N - 1] = 0;
                b[N - 1] = 1;
            ...
            else
            {
                ...
                a[N - 1] = -coeff;
                b[N - 1] = (Real(1.0) + Real(2.0) * coeff) - (-coeff); // 1+3coeff
                c[N - 1] = 0.0;
                ...
                Real du_ex = 0.0;
                if (comp == 1)
                    du_ex = ub_delta(1, Lx, (Real(j) + Real(0.5)) * dy, z);
                else
                    du_ex = ub_delta(2, Lx, y, (Real(k) + Real(0.5)) * dz);

                d[N - 1] = rhs_delta.value(comp, N - 1, j, k) + Real(2.0) * coeff * du_ex;
            }

            thomas_algorithm(a, b, c, d, x);

            for (Dim i = 0; i < N; ++i)
                delta.set(comp, i, j, k) = x[i];
        };

#ifdef _OPENMP
        if (use_omp)
        {
#pragma omp parallel for collapse(2) default(none) shared(solve_line)
            for (Dim k = 0; k < Nz; ++k)
                for (Dim j = 0; j < Ny; ++j)
                {
                    solve_line(j, k, 0);
    ...
    void solve_y_only_delta(VectorVariable &rhs_delta,
                            VectorVariable &delta,
                            Real t_old,
                            Real t_stage,
                            bool use_omp = false)
    {
        ...
        auto solve_line = [&](Dim i, Dim k, int comp)
        {
            std::vector<Real> a(N, 0.0), b(N, 0.0), c(N, 0.0), d(N, 0.0), x(N, 0.0);
            ...
            for (Dim j = 1; j < N - 1; ++j)
            {
                ...
                b[j] = Real(1.0) + Real(2.0) * coeff;
                c[j] = -coeff;
                d[j] = rhs_delta.value(comp, i, j, k);
            }
            ...
            // y=0
            {
                ...
                if (comp == 1)
                {
                    const Real dv0 = ub_delta(1, x_vw, Real(0.0), z_u);

                    const Real du_plus = ub_delta(0, x_u + Real(0.5) * dx, Real(0.0), z_u);
                    const Real du_minus = ub_delta(0, x_u - Real(0.5) * dx, Real(0.0), z_u);
                    const Real ddu_dx = (du_plus - du_minus) / dx;

                    const Real dw_plus = ub_delta(2, x_vw, Real(0.0), z_w + Real(0.5) * dz);
                    const Real dw_minus = ub_delta(2, x_vw, Real(0.0), z_w - Real(0.5) * dz);
                    const Real ddw_dz = (dw_plus - dw_minus) / dz;

                    const Real ddvdy0 = -(ddu_dx + ddw_dz);
                    d[0] = dv0 + ddvdy0 * (dy * Real(0.5));
                }
                else if (comp == 0)
                {
                    d[0] = ub_delta(0, x_u, Real(0.0), z_u);
                }
                else
                {
                    d[0] = ub_delta(2, x_vw, Real(0.0), z_w);
                }
            }
            ...
            else
            {
                ...
                a[N - 1] = -coeff;
                b[N - 1] = (Real(1.0) + Real(2.0) * coeff) - (-coeff);
                c[N - 1] = 0.0;
                ...
                Real du_ex = 0.0;
                if (comp == 0)
                    du_ex = ub_delta(0, x_u, Ly, z_u);
                else
                    du_ex = ub_delta(2, x_vw, Ly, z_w);

                d[N - 1] = rhs_delta.value(comp, i, N - 1, k) + Real(2.0) * coeff * du_ex;
            }

            thomas_algorithm(a, b, c, d, x);

            for (Dim j = 0; j < N; ++j)
                delta.set(comp, i, j, k) = x[j];
        };
...
#pragma omp parallel for collapse(2) default(none) shared(solve_line)
    ...
    void solve_z_only_delta(VectorVariable &rhs_delta,
                            VectorVariable &delta,
                            Real t_old,
                            Real t_stage,
                            bool use_omp = false)
    {
        ...
        auto solve_line = [&](Dim i, Dim j, int comp)
        {
            std::vector<Real> a(N, 0.0), b(N, 0.0), c(N, 0.0), d(N, 0.0), x(N, 0.0);
            ...
            for (Dim k = 1; k < N - 1; ++k)
            {
                ...
                b[k] = Real(1.0) + Real(2.0) * coeff;
                c[k] = -coeff;
                d[k] = rhs_delta.value(comp, i, j, k);
            }
            ...
            // z=0
            {
                ...
                if (comp == 2)
                {
                    const Real dw0 = ub_delta(2, x_vw, y_u, Real(0.0));

                    const Real du_plus = ub_delta(0, x_u + Real(0.5) * dx, y_u, Real(0.0));
                    const Real du_minus = ub_delta(0, x_u - Real(0.5) * dx, y_u, Real(0.0));
                    const Real ddu_dx = (du_plus - du_minus) / dx;

                    const Real dv_plus = ub_delta(1, x_vw, y_v + Real(0.5) * dy, Real(0.0));
                    const Real dv_minus = ub_delta(1, x_vw, y_v - Real(0.5) * dy, Real(0.0));
                    const Real ddv_dy = (dv_plus - dv_minus) / dy;

                    const Real ddwdz0 = -(ddu_dx + ddv_dy);
                    d[0] = dw0 + ddwdz0 * (dz * Real(0.5));
                }
                else if (comp == 0)
                {
                    d[0] = ub_delta(0, x_u, y_u, Real(0.0));
                }
                else
                {
                    d[0] = ub_delta(1, x_vw, y_v, Real(0.0));
                }
            }
            ...
            else
            {
                ...
                Real du_ex = 0.0;
                if (comp == 0)
                    du_ex = ub_delta(0, x_u, y_u, Lz);
                else
                    du_ex = ub_delta(1, x_vw, y_v, Lz);

                d[N - 1] = rhs_delta.value(comp, i, j, N - 1) + Real(2.0) * coeff * du_ex;
            }

            thomas_algorithm(a, b, c, d, x);

            for (Dim k = 0; k < N; ++k)
                delta.set(comp, i, j, k) = x[k];
        };
