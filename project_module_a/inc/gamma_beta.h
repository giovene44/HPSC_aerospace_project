...
public:
    ...
    void initialized_index(const Matrix3D<float> &k, float delta_t, float nu, int i, int j, int k_idx);
    /**
...
void Gamma::initialized_index(const Matrix3D<float> &k, float delta_t, float nu, int i, int j, int k_idx)
{
...
void Gamma::initialize(const Matrix3D<float> &k, float delta_t, float nu)
{
    ...
    for (size_t i = 0; i < dim_x_; ++i)
    {
        for (size_t j = 0; j < dim_y_; ++j)
        {
            ...
            {
                this->initialized_index(k, delta_t, nu, i, j, k_idx);
            }
        }
    }
}
