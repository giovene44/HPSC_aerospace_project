    ...
    for (Dim k = 0; k < g.Nz; ++k)
        for (Dim j = 0; j < g.Ny; ++j)
            for (Dim i = 0; i < g.Nx; ++i)
            {
                ...
                Real sd = scalar.second_derivative(0, i, j, k);
                Real rhs_val = val - sd;
...
bool solve_and_check(PressureSolver &solver, ScalarVariable &rhs,
                     ...
                     Real &l2_error, double &elapsed_seconds,
                     bool use_omp)
{
    ...
    solver.solve_pressure<0>(rhs, computed_sol, x_handler, use_omp);
...
int main()
{
    ...
    printf("=================================================\n");
    printf("X-DIRECTION PRESSURE SPLITTING CONVERGENCE STUDY\n");
    printf("=================================================\n\n");
    ...
    for (Dim N : grid_sizes)
    {
        ...
        bool success_serial = solve_and_check(solver_serial, rhs_serial, scalar, g, x_handler, l2_error_serial, t_serial, /*use_omp=*/false);
        bool success_parallel = solve_and_check(solver_parallel, rhs_parallel, scalar, g, x_handler, l2_error_parallel, t_parallel, /*use_omp=*/true);
        ...
        speed_ups.emplace_back(speedup);
        ...
        if (success_serial && success_parallel)
    ...
    printf("=================================================\n");
    printf("Results written to: convergence_pressure_x.txt\n");
