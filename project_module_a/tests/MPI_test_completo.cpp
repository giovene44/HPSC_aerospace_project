...
// Vector field synchronization: all ranks exchange complete arrays for all 3 components
// This is inefficient but correct for debugging
static void sync_vector_field_full(VectorVariable &field, const Grid &g,
                                   const MPITopology3D &topo)
{
#ifdef USE_MPI
    MPI_Comm comm = topo.cart_comm();
    ...
    // Total number of elements per component
    const int total = g.Nx * g.Ny * g.Nz;
    ...
    // Synchronize each component separately
    for (int comp = 0; comp < 3; ++comp)
    {
        // Pack local field data for this component
        std::vector<Real> local_data(total);
        for (Dim k = 0; k < g.Nz; ++k)
            for (Dim j = 0; j < g.Ny; ++j)
                for (Dim i = 0; i < g.Nx; ++i)
        ...
        else
        {
            ...
            for (Dim k = k0; k < k1; ++k)
                for (Dim j = j0; j < j1; ++j)
                    for (Dim i = i0; i < i1; ++i)
                        send_buf[idx++] = field.value(comp, i, j, k);
                                       ...
                                       ErrorRegion region, const MPITopology3D &topo)
{
    ...
    for (int n = 0; n < nsteps; ++n)
    {
        ...
        sync_vector_field_full(u_tmp, g, topo);  // Sync u_tmp for Y-solve

        solver.solve_y_only(u_tmp, u_np1, topo, false);
        sync_vector_field_full(u_np1, g, topo);  // Sync u_np1 for Z-solve

        solver.solve_z_only(u_np1, u_tmp, topo, false);
        sync_vector_field_full(u_tmp, g, topo);  // Sync final velocity
...
int main(int argc, char **argv)
{
    ...
    // Scope block ensures MPITopology3D is destroyed BEFORE MPI_Finalize
    {
        ...
        {
            std::cout << "=================================================\n";
            std::cout << "MPI_TEST_COMPLETO: Navier-Stokes con pressione MPI\n";
            std::cout << "Processori: " << world_size << " (" << Px << "x" << Py << "x" << Pz << ")\n";
            std::cout << "=================================================\n";
        }
        ...
        // 30 timestep con dt=8e-5 => Tfinal = t0 + 30*dt = 0.15 + 0.0024 = 0.1524
        const Real Tfinal = t0 + Real(30) * Real(8e-5);

        test_refine_dx(nu, k, t0, Tfinal, topo);
    } // topo destroyed here, before MPI_Finalize
