    ...
    for (int c = 0; c < 3; ++c)
        for (Dim k = 1; k < g.Nz - 1; ++k)
            for (Dim j = 1; j < g.Ny - 1; ++j)
                ...
                {
                    Real lap =
                        (u.value(c, i, j + 1, k) - 2 * u.value(c, i, j, k) + u.value(c, i, j - 1, k)) / (g.dy * g.dy);
    ...
    bool parallel)
{
    ...
    for (int n = 0; n < nsteps; ++n)
    {
        ...
        // X sweep
        solver.solve_y_only(rhs, u_np1, parallel);
...
int main()
{
    ...
    std::ofstream outfile("test_navier_stokes_y_direction.txt");
    ...
    printf("=================================================\n");
    printf("Results written to test_navier_stokes_y_direction.txt\n");
